...
export type Command<TOptions extends CommandOptions = CommandOptions> =
	CreateApplicationCommand & {
		...
		run: (
			...
			options: GetCommandOptions<TOptions>,
		) => unknown;
...
export type GetCommandOptions<T extends CommandOptions> =
...
type ResolvedValues = ParsedInteractionOption<
	ExtractDesiredProps<typeof bot>,
	ExtractDesiredBehavior<typeof bot>
>[string];
...
export type InteractionResolvedChannel = Omit<
	...
	Exclude<
		keyof typeof bot.transformers.$inferredTypes.channel,
		"id" | "name" | "type" | "permissions" | "threadMetadata" | "parentId"
	>
>;
export type InteractionResolvedMember = Omit<
...
interface TypeToResolvedMap {
	...
	[ApplicationCommandOptionTypes.Mentionable]:
		| typeof bot.transformers.$inferredTypes.role
		| InteractionResolvedUser;
	[ApplicationCommandOptionTypes.Number]: number;
	[ApplicationCommandOptionTypes.Attachment]: typeof bot.transformers.$inferredTypes.attachment;
}

type ConvertTypeToResolved<T extends ApplicationCommandOptionTypes> =
	T extends keyof TypeToResolvedMap ? TypeToResolvedMap[T] : ResolvedValues;
...
type GetOptionValue<T> = T extends {
	...
	? T extends { type: SubCommandApplicationCommand; options?: CommandOptions }
		...
		:
				| ConvertTypeToResolved<T["type"]>
				| (T["required"] extends true ? never : undefined)
	: never;

type BuildOptions<T extends CommandOptions | undefined> = {
	[Prop in keyof Omit<T, keyof unknown[]> as GetOptionName<
		T[Prop]
	>]: GetOptionValue<T[Prop]>;
};

type ExtractDesiredProps<T> = T extends Bot<infer Props, infer _Behavior>
	? Props
	: never;
type ExtractDesiredBehavior<T> = T extends Bot<infer _Props, infer Behavior>
	? Behavior
	: never;
