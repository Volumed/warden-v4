...
async function connectToRabbitMQ(): Promise<void> {
	const connection = await connectAmqp(
		`amqp://${MESSAGEQUEUE_USERNAME}:${MESSAGEQUEUE_PASSWORD}@${MESSAGEQUEUE_URL}`,
	).catch((error) => {
	...
	connection.on("error", (error) => {
		bot.logger.error(
			"There was an error in the connection with RabbitMQ, reconnecting in 1s.",
			error,
		);
		setTimeout(connectToRabbitMQ, 1000);
	});
	...
	await channel
		.bindQueue("gatewayMessageQueue", "gatewayMessage", "")
	...
	await channel
		.consume("gatewayMessageQueue", async (message) => {
			...
			try {
				...
				bot.logger.error(
					"There was an error handling events received from RabbitMQ",
					error,
				);
			}
		})
